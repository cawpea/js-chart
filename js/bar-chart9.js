...
var CHART = {
	...
	bindEvent : function(){
		/*
		 * リサイズが発生した時に実行するイベントハンドラを設定する
		 */
		this.$window.on('resize', $.proxy(this.watchResize, this));
	},
	/*
	 * リサイズ時には、これ以降のメソッドを再実行する
	 */
	...
	setupCommonParameters : function(){
		/*
		 * canvasのサイズは初期化時だけではなく、再描画時にも設定し直す
		 */
	...
	watchResize : function(event){
		/*
		 * リサイズが発生した時に、既にタイマーが設定されている場合は、全て停止させる
		 */
		 ...
		 * 改めてタイマーを設定する
		 */
		this.timerIds.push(requestAnimationFrame($.proxy(this.redrawChart, this)));
	},
	redrawChart : function(){
		var width = this.$container.width();
		
		/*
		 * 前回と幅が異なる場合はリサイズ中であると判定し、再度タイマーを設定する
		 */
		if(this.width != width){
			this.width = width;
			this.watchResize();
		...
		/*
		 * 前回と幅が同じ場合はリサイズが終了したものと判定し、チャートを再描画する
		 */
		this.setupCanvas();
	},
