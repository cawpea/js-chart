...
var CHART = {
	...
	/*
	 * スクロールの位置を監視し、チャートが表示領域に入った時にアニメーションを開始する
	 * このメソッドは初期表示時と、スクロールイベント発生時に実行される
	 */
	watchScroll : function(event){
		...
		/*
		 * チャートの高さ <= ウィンドウの高さ : チャートが表示領域に完全に含まれた時にアニメーションを開始する
		 * チャートの高さ > ウィンドウの高さ : 表示領域がチャートと完全に重なった時にアニメーションを開始する
		 */
		if(containerTop >= windowTop && containerBottom <= windowBottom || containerTop <= windowTop && containerBottom >= windowBottom){
			/*
			 * 一度アニメーションを実行したら、スクロールイベントハンドラを解除する
			 */
